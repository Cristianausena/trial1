    ...
    if request.user.is_authenticated and request.user.usertype == 'ADMIN':
        ...
        if request.method == "POST":
            ...
            if actions == "ACCEPT":
                ...
                user2 = accounts.objects.get(idnumber=vehicle_status_update.idnumber)
                print(user2.email)
                vehicle_status_update.status = "ACCEPTED"

                send_mail(
                'Confirmation',
                'Good Day! Your vehicle application (' + str(vehicle_status_update.vehicleid) + ') has been approved. Visit your logbook portal for QR Code',
                ...
                vehicle_status_delete = registered_vehicles.objects.get(vehicleid=ids)
                user2 = accounts.objects.get(idnumber=vehicle_status_delete.idnumber)


                send_mail(
                'Confirmation',
                'Good Day! Your vehicle application (' + str(vehicle_status_delete.vehicleid) + ') has been declined.',
                'tropaniaubrey@gmail.com',
                [user2.email],)
        ...
        context = {'vehicle_image': vehicle_image, 'user': user, 'user2': user2}
